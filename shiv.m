function [beta0_final,beta_final,beta_scale,glmstats] = ...
    shiv(X,M,y,r,dist,lambda,pentype,penparam,varargin)

%4.2.2018
%Xuan:
%Create a sparsereg function for the proposed method
%where lambda is pre-specified as dynamic rather than a constant
...
%         %bsxfun(@func,A,B): element-wise operation of @func on A and B
%         %in the above case, a column of beta.U{j} is treated as an element
%     end
...
%                     *khatrirao({beta.U{d:-1:j+1},cumkr}),n,p(j)*r);
                Xj = reshape(double(tenmat(TM,[d+1,j])) ...
                    *khatrirao([beta.U(d:-1:j+1),cumkr]),n,p(j)*r); 
                %just in case Md doesn't exist
                %this line is revised. The original one may be wrong.
                %However, using [] is not mentioned in khatrirao.m
            end
        end
        if (isglm)
            ...
            if strcmpi(pentype,'B1XB2XB3') %B1XB2XB3 represents IV as in the paper
%            if strcmpi(pentype,'fiberlasso')
                ...
                TCV(j,:)=[];
                LW=prod(TCV,1);
%                LW=prod(TCV,1)./TCV(j,:); %lambda weights, 1-by-r
            else
                LW=ones(1,r);
            end
            betatmp = lsq_sparsereg([Xj,eta0],y,lambda,'lambda_weights',LW,'weights',wts,...
...
%         - r*(d-1),0);
    glmstats{d+1}.BIC = dev0 + log(n)* ...
        max(sum(arrayfun(@(j) nnz(unique(reshape(beta.U{j},[],1))), 1:d, 'UniformOutput',true)) ...
        - r*(d-1),0);
end
